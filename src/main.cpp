...
////            either we will referencing the starting point as where the car is or at the previous paths end point
//                    double ref_x = car_x;
//                    double ref_y = car_y;
...
//
////            Define the actual (x,y) values we will use for the planner
...
//                    double x_add_on = 0;
//
////            Fill the rest of our path planner after filling it with the previous points, here we will always output 50 points
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    // Take a look for other cars
                    for(int i = 0; i < sensor_fusion.size(); i++){
                        //check for cars in my lane
                        float d = sensor_fusion[i][6];
                        if (d < 0){
                            continue;
                        }
                        ...
                        //predict the car future s for the time my car is in the last s point
                        ...
                        if(d < (2+4*lane+2) && d > (2+4*lane-2)){
                            ...
                            //check s values greater than mine and s gap
                            car_left |= car_s - 20 < check_car_s && car_s + 40 > check_car_s;
                        }else if(d > (2+4*lane) && d < (2*4*(lane+1)+2)){
                            //check s values greater than mine and s gap
                            car_right |= car_s - 20 < check_car_s && car_s + 40 > check_car_s;
                        }
                    }
                    ...
                    if (prev_size < 2){
                        // if the previous path is almost empty, use the car as the starting point
                        double prev_car_x = car_x - cos(car_yaw);
                        double prev_car_y = car_y - sin(car_yaw);
                        ...
                        // if the car has some point to follow get the last 2 ones
                        ref_x = previous_path_x[prev_size - 1];
                        ref_y = previous_path_y[prev_size - 1];
